function [LLH, logL] = VariationVarianceLogLikelihoodControls(lambdabar, tauhat, betap, cutoffs, symmetric, X, sigma,C,numerical_integration,symmetric_p)
...
if symmetric==1
    for m=1:length(cutoffs)
        ...
        if numerical_integration==1
            ...
            g=@(theta)(normcdf(abs(cutoffs(m))-theta./sigma)-normcdf(-abs(cutoffs(m))-theta./sigma))...
                .*gampdf(theta,lambdabar,tauhat);
            prob_vec(:,m+1)=integral(g,-inf,inf,'Arrayvalued',true); 
        else
            %Monte Carlo Integration
            g=abs((normcdf(cutoffs(m)-theta_mat./sigma_mat)...
