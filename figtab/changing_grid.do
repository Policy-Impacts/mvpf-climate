...
tw ///
	(line MVPF percent if category == "wind") ///
	(line MVPF percent if category == "solar") ///
	(line MVPF percent if category == "solar no lbd", lp(dash)) ///
	(line MVPF percent if category == "wind no lbd", lp(dash)) ///
	(line MVPF percent if category == "weatherization") ///
	(line MVPF percent if category == "appliance rebates") ///
	(line MVPF percent if category == "ev") ///
	, ///
	xline(`percent_today', noextend lcolor("black") lpattern(shortdash)) ///
	graphregion(color(white)) legend(order(1 "Wind" 2 "Solar" 3 "Solar No LBD" 4 "Wind No LBD" 5 "Weatherization" 6 "Appliance Rebates" 7 "EVs")) ///
