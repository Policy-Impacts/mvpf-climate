...
****************************************************
	...
	** Cost assumptions
	...
	if "${spec_type}" == "baseline" {
		...
		local pre_cost_per_watt = 3.43 * (${cpi_${policy_year}}/${cpi_2012}) // $/W (Table B.1) - Assuming they are using a dollar year from the middle of their sample
		local avg_state_rebate = 0.41 * (${cpi_${policy_year}}/${cpi_2012}) // $/W, Table B.1 - Assuming they are using a dollar year from the middle of their sample
	...
	local e_demand = ((`price' + (2 * 3.7 * `price_squared')) * 3.7)/0.518
	local semie = `e_demand'/`cost_per_watt'
	
	local pass_through1 = 0.778 // From Pless Paper
	local pass_through2 = 0.75 // From Pless Paper (additional pass through for TPO)
