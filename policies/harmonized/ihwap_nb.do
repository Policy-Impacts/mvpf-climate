...
weatherization_ado non-marginal, policy_year(${policy_year}) inflation_year(2017) spec(${emissions_factor}) geo(${State}) kwh_reduced(`kwh_reduction_annual') mmbtu_reduced(`mmbtu_reduction_annual') program_cost(`retrofit_cost') replacement(`replacement') policy("ihwap_nb")
di `r(MVPF)'
