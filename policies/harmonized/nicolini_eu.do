...
*For Semie
local capital_discount = 0.0397 // https://www.nrel.gov/docs/fy24osti/88335.pdf, Slide 50 (Real Weighted Avg Cost of Capital (%))
local ptc_discount_rate = 0.0673 // https://www.nrel.gov/docs/fy24osti/88335.pdf, Slide 50 (Real Fixed Charge Rate (%))
...
/* 5. Outputs */
****************
wind_ado, policy_year(${policy_year}) inflation_year(${policy_year}) spec(${spec_type}) replacement(`replacement') capacity_factor_context(`capacity_factor_context') size_context(`average_size') semie(`semie') p_name("nicolini_eu") marg_sales(`marg_sales') cum_sales(`cum_sales') prod_cost(`prod_cost') epsilon(`epsilon') farmer_theta(`farmer_theta') subsidy_max(`subsidy_max') current_ptc(`current_ptc')
