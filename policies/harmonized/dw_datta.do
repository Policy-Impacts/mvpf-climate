*************************************************************************************
/*       0. Program: Energy Efficient Appliance Rebate -- Dishwashers        */
*************************************************************************************
...
if "${incr_appliance_lifetimes}" == "yes" {
	"appliance_lifetimes" = 18
}

if "${decr_appliance_lifetimes}" == "yes" {
	"appliance_lifetimes" = 13
}
...
local appliance_cost = (543+84) * (${cpi_`dollar_year'}/${cpi_2010}) // From Aldy Paper Table 2
local avg_rebate = 34.348 * (${cpi_`dollar_year'}/${cpi_2004}) // Table 3
// local annual_kwh = ((700+700+574+439+413+448)/6) - ((555+555+455+336+341+341)/6) // kwh per year (Table 7)
...
local epsilon = `semie' * (((543+84) * (${cpi_${policy_year}}/${cpi_2010})) - (34.348 * (${cpi_${policy_year}}/${cpi_2004})))
...
local dishwasher_energy_savings = `annual_e_savings' + (`annual_e_savings' / `discount') * (1 - (1 / (1 + `discount')^(`lifetime' - 1)))
