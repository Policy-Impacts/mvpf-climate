...
local ptc_2020 = `current_ptc' + ((`current_ptc')/`ptc_discount_rate') * (1 - (1/(1+`ptc_discount_rate')^(`credit_life' - 1)))
...
local semie = `semie_paper' * ((`lcoe_discounted_incontext' - `ptc_incontext') / (`lcoe_discounted' - `ptc_2020'))
...
if "${constant_semie}" == "yes" {
	local ratio_adjusted = `ptc_discounted' / (`lcoe_discounted' - `ptc_2020')
	local semie = `semie_paper'
...
/* 5. Outputs */
****************
wind_ado, policy_year(${policy_year}) inflation_year(${policy_year}) spec(${spec_type}) replacement(`replacement') capacity_factor_context(`capacity_factor_context') size_context(`average_size') semie(`semie') p_name("hitaj_ptc") marg_sales(`marg_sales') cum_sales(`cum_sales') prod_cost(`prod_cost') epsilon(`epsilon_cost_curve') farmer_theta(`farmer_theta') subsidy_max(`subsidy_max') current_ptc(`current_ptc')
