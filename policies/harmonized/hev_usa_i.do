...
preserve
	...
	* Consumer price = includes taxes. 
	...
	if "${spec_type}" == "baseline" {
		...
		keep if state == "Colorado" | state == "Maryland" | state == "New York" | state == "Oregon" | state == "Pennsylvania" | state == "South Carolina" | state == "Utah" | state == "West Virginia"
	...
	** avg over policy's time period (2000-2006) and across states
	if "`4'" == "baseline"{
		...
		forvalues y = 2000(1)2006{
			...
			qui sum pop`y' if state == "New York"
			local NY_pop_`y' = r(mean)
			...
			qui sum pop`y' if state == "West Virginia"
			local WV_pop_`y' = r(mean)
		}
		keep if inlist(state, "Colorado", "Maryland", "New York", "Oregon", "Pennsylvania", "South Carolina", "Utah", "West Virginia")
...
preserve
	if "`4'" == "baseline"{
		...
		keep if state == "CO" | state == "MD" | state == "NY" | state == "OR" | state == "PA" | state == "SC" | state == "UT" | state == "WV"
...
preserve
	if "`4'" == "baseline"{
		...
		keep if state == "CO" | state == "MD" | state == "NY" | state == "OR" | state == "PA" | state == "SC" | state == "UT"
		keep state age vmt_by_state_age
		gen population = .
		foreach s of local states{
			local state_pop = 0
